...
class AdvEvaluator:
  ...
  def run_evaluation(
    ...
    sort_indices = np.argsort(reward)
    ...
    # del eval_metrics['reward']
    # for k,v in eval_metrics.episode_metrics.items():
    #   clean_key = k.replace('/', '_')
    #   metrics[f'eval/{clean_key}'] = np.mean(v)
    #   for label, idx in percentiles.items():
    #     # Ensure index doesn't exceed array bounds for small N
    #     safe_idx = min(idx, N - 1)
        
    #     # Get the index of the episode that hit that reward percentile
    #     episode_idx = sort_indices[safe_idx]
        
    #     # Save the value of the metric 'k' for that specific episode
    #     metrics[f'eval/{clean_key}_{label}'] = v[episode_idx]

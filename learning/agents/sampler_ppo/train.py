    ...
    if use_scheduling:
      ...
      if scheduler_mode=="linear":
        ...
        _beta = start_beta - (start_beta - end_beta) * (1 - jnp.exp(-k * training_state.update_steps / num_training_steps))/ (1  - jnp.exp(-training_state.update_steps))
          ...
          values75 = jnp.percentile(values, 75)
          values50 = jnp.percentile(values, 50)
          values25 = jnp.percentile(values, 25)
          ...
          _beta = jnp.where(values75 > .5, 0., _beta)
          _beta = jnp.where(values50 > .5, (sb + eb) / 2., _beta)
          _beta = jnp.where(values25 > .5, eb, _beta)

...
_CONFIG_FNAME = 'flowppo_network_config.json'
...
def network_config(
    ...
    network_factory: types.NetworkFactory[flowppo_networks.FLOWPPONetworks],
...
def _get_flowppo_network(
    config: config_dict.ConfigDict,
    network_factory: types.NetworkFactory[flowppo_networks.FLOWPPONetworks],
) -> flowppo_networks.FLOWPPONetworks:
  """Generates a FLOWPPO network given config."""
...
def load_config(
  ...
  """Loads FLOWPPO config from checkpoint."""
...
def load_policy(
  ...
  """Loads policy inference function from FLOWPPO checkpoint."""
  path = epath.Path(path)
  config = load_config(path)
  params = load(path)
  flowppo_network = _get_flowppo_network(config, network_factory)
  make_inference_fn = flowppo_networks.make_inference_fn(flowppo_network)

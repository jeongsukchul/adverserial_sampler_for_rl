...
from agents.sampler_ppo import checkpoint
...
from learning.module.normalizing_flow.simple_flow import render_flow_pdf_2d_subplots
...
from learning.module.wrapper.evaluator import AdvEvaluator, Evaluator, generate_adv_unroll
...
def train(
    ...
    dr_train_ratio = 1.0,
    use_wandb= False,
    sampler_update_freq=20,
    alpha = 0.5,
    beta = 1.0,
    tau = 1.0,
    sampler_choice : int = 0., #(0 : Uniform distribution 1: GOFLOW sampler, 2: flow sampler, 3: GMM sampler)
    n_sampler_iters : int = 10.,
):
  """SAMPLERPPO training.
  ...
  Args:
    ...
    randomization_fn: a user-defined callback function that generates randomized
      environments
    learning_rate: learning rate for samplerppo loss
    entropy_cost: entropy reward for samplerppo loss, higher values increase entropy of
      the policy
    discounting: discounting rate
    unroll_length: the number of timesteps to unroll in each environment. The
      SAMPLERPPO loss is computed over `unroll_length` timesteps
    ...
    clipping_epsilon: clipping epsilon for SAMPLERPPO loss
    ...
    restore_params: raw network parameters to restore the TrainingState from.
      ...
      from the return values of samplerppo.train().
  ...
  key_policy, key_wrap, key_value, gmm_key = jax.random.split(global_key, 4)
  del global_key
  ...
  gmm_batch_size= 1024#batch_size * num_minibatches
  if normalize_observations:
    normalize_fn = running_statistics.normalize
  samplerppo_network, init_gmm_state = network_factory(
  ...
  goflowloss_fn = functools.partial(
    samplerppo_losses.goflow_loss,
    samplerppo_network=samplerppo_network,
    dr_range_high=dr_range_high,
    dr_range_low=dr_range_low,
    alpha=alpha,
  ...
  goflow_update_fn = gradients.gradient_update_fn(
    goflowloss_fn, flow_optimizer, pmap_axis_name=_PMAP_AXIS_NAME, has_aux=True
  )
  ...
  def training_step(
    ...
    key_sgd, key_generate_unroll, param_key, key_gmm, key_flow, new_key = jax.random.split(key, 6)
    
    dynamics_params, mapping = samplerppo_network.sampler_network.sample_selector.select_samples(\
              training_state.sampler_training_state.model_state, param_key)
    # #for debugging
    ...
    def update_flow():
      self_normalized_values = (cumulated_values - cumulated_values.mean())/(cumulated_values.mean() +1e-8 ) 
      target_lnpdf = jnp.log(self_normalized_values)
      def body(carry, _):
        params, optimizer_state = carry
        (_, flow_metrics), params, optimizer_state = goflow_update_fn(
            params,
            dynamics_params,
            target_lnpdf,
            key_flow,
      ...
      (params, optimizer_state), _  = jax.lax.scan(body, (params, optimizer_state), (), length=n_sampler_iters)
      return params, optimizer_state, 1.
    def update_goflow():
      self_normalized_values = (cumulated_values - cumulated_values.mean())/(cumulated_values.mean() +1e-8 ) 
      target_lnpdf = jnp.log(self_normalized_values)
      def body(carry, _):
    ...
    def update_gmm():
      self_normalized_values = (cumulated_values - cumulated_values.mean())/(cumulated_values.mean() +1e-8 ) 
      target_lnpdf = jax.nn.log_softmax(-tau * self_normalized_values,axis=-1)  # [num_envs]
      new_sample_db_state = samplerppo_network.gmm_network.sample_selector.save_samples(training_state.gmm_training_state.model_state, \
                    training_state.gmm_training_state.sample_db_state, dynamics_params, target_lnpdf, \
      ...
      new_gmm_training_state = gmm_update_fn(new_gmm_training_state, key_gmm)

      return new_gmm_training_state, 1.
    if sampler_choice==0:
      None
    elif sampler_choice==1:
      optimizer_state, params, update_signal = jax.lax.cond(i % sampler_update_freq==0, \
                lambda: update_goflow(), \
                 lambda: (optimizer_state, params, 0.) )
  ...
  if process_id == 0 and num_evals > 1 and run_evals:
    ...
    if sampler_choice == 1 or sampler_choice ==2:
      fig = render_flow_pdf_2d_subplots(
        samplerppo_network.flow_network,
      ...
      sample_key1, sample_key2, local_key = jax.random.split(local_key, 3)
      samples = samplerppo_network.gmm_network.model.sample_from_components_no_shuffle(_unpmap(\
        training_state.gmm_training_state.model_state.gmm_state), 1000, \
          _unpmap(training_state.gmm_training_state.model_state.gmm_state.num_components), sample_key1)[0]
  ...
  for it in range(num_evals_after_init):
    ...
    if num_evals > 0:
      ...
      if sampler_choice==3:
        sample_key1, sample_key2, local_key = jax.random.split(local_key, 3)
        samples = samplerppo_network.gmm_network.model.sample_from_component_no_shuffle(_unpmap(\
          training_state.gmm_training_state.model_state.gmm_state), 1000, \
            _unpmap(training_state.gmm_training_state.model_state.gmm_state.num_components), sample_key1)[0]
        eval_samples = samplerppo_network.gmm_network.model.sample(_unpmap(\
          training_state.gmm_training_state.model_state.gmm_state), sample_key2, 1000)[0]

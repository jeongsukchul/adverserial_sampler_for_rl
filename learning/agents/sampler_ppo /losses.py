...
from agents.samplerppo import networks as samplerppo_networks
...
def goflow_loss(
  ...
  data_log_prob = samplerppo_network.flow_network.apply(
      params.flow,
      low=dr_range_low,
      high=dr_range_high,
      mode='log_prob',
  ...
  _, current_logp = samplerppo_network.flow_network.apply(
      params.flow,
      mode='sample',
      low=dr_range_low,
      high=dr_range_high,
  ...
  _, current_logp_on_prev = samplerppo_network.flow_network.apply(
      params.flow,
      low=dr_range_low,
      high=dr_range_high,
      mode='log_prob',
  ...
  kl_loss = prev_logp - current_logp_on_prev
  # Get next action and log prob from policy for adversarial observation
  if entropy_update:
    value_loss = - volume * (data_log_prob *jnp.exp(data_log_prob) * jnp.exp(target_lnpdf)).mean()
    entropy_loss = volume * (jnp.exp(current_logp) * current_logp).mean()
  else:
    value_loss = - (data_log_prob * jnp.exp(target_lnpdf)).mean()
    entropy_loss = (volume * jnp.exp(current_logp) * current_logp).mean()
  ...
  return  value_loss + alpha *  entropy_loss + beta * kl_loss, {
